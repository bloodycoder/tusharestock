...
ts.set_token(token)
ftoken.close()
flog =  open('log.txt', 'a',encoding='utf-8')
...
for stock_code, points in sorted_data:
    ...
    for i in range(5, 10):
        log += "year " + str(i) + " roe:\n"
        buypoint = float(jingzichan * ((1+(roe_low/100))**i)) - jingzichan*(1-youzhi_rate)
        sellpoint = float(jingzichan * ((1+(roe_high/100))**i)) - jingzichan*(1-youzhi_rate)
        log += " buypoint:" + '{:.3f}'.format(buypoint) + " sellpoint: " + '{:.3f}'.format(sellpoint) + "\n"
    log += "-------------------------\n"
flog.write(log)
